...
if user_input:
    ...
    # 2 — status accordion + routing
    with st.status("🐺 Wolf Scholar is working...", expanded=True) as status_box:
        try:
            result = route(user_input, uploaded_file, vs_ref)
            rtype  = result["type"]
            status_box.update(label=STATUS_LABELS[rtype], expanded=True)
        except Exception as e:
            status_box.update(label="⚠️ Routing error", state="error", expanded=True)
            st.error(str(e))
            st.stop()
    ...
    with st.chat_message("assistant"):
        try:
            if rtype == "image":
                st.image(result["content"], caption="Generated by Pollinations AI · flux model")
                ...
                # Non-streaming (full string)
                full_response = result["content"]
                st.markdown(full_response)

            else:
                # Streaming (rag or search)
                full_response = st.write_stream(result["content"])

            # Agent badge
            badge = result["label"]
            st.markdown(
                f'<span class="agent-badge badge-{rtype}">{badge}</span>',
                unsafe_allow_html=True,
            )
            ...
            if "429" in err or "rate_limit" in err.lower():
                ...
                full_response = f"⚠️ Error: {err}"
            st.error(full_response)

    # Update status to done
    with st.status(STATUS_DONE.get(rtype, "✅ Done"), state="complete", expanded=False):
        pass
    ...
    st.session_state.ws_upload = None
